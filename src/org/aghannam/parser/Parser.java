...
/**
 * This class implements a (predictive) recursive-descent parser for XML--, a fictional subset of XML. 
 * <p>
 ...
 * which case it prints out a leftmost derivation with one grammar rule displayed per line. Essentially, the parser 
 * simulates a deterministic pushdown automaton that is equivalent to the above grammar. Per the grammar, the tokens are 
 * NAME, DATA, STRING, <, >, &lt;/, /&gt;, and =. 
 * <p>
...
public class Parser {
	...
	private ArrayList<Token> tokens; // the token stream returned by the lexical analyzer 
	private Stack<String> tagNames;  // used to hold tag names to ensure that they match where necessary 
	private HashSet<String> attributeNames; // used to hold attribute names to ensure no duplicates 
	private Token lookahead; // the lookahead symbol containing the next token in the input stream 
	
	/**
	 * Parses an XML-- document using recursive descent and prints a leftmost derivation that 
	 * corresponds to a parse tree generating the given input token sequence. 
	 *  
	...
	public void parse(List<Token> tokens) throws ParserException {
		try {
			...
			// An IndexOutOfBoundsException may be triggered if the parser detects something highly unusual within
	...
	/**
	 * Represents the grammar rule 'document ::= element EOF'. 
	 * 
	...
	/**
	 * Represents the grammar rule 'elementPrefix ::= NAME attribute elementSuffix'. 
	 * <p>
	 * Since this rule contains a name, this method calls <code>cacheTagName()</code> 
	 * to store tag-opening names should a name-matching with an end tag be necessary. In addition, 
	 * because it also potentially contains attributes, it delegates attribute name-checking to 
	 * <code>attribute()</code> which is called from within this method and ensures that any attributes
	 * defined within the current tag have no duplicate names. 
	 * 
	 * @throws ParserException if a syntax error is encountered 
	 */
	...
	/**
	 * Represents the grammar rules 'attribute ::= NAME = STRING attribute' and 'attribute ::= EPSILON'
	 * <p>
	 * As the number of attributes appearing within a tag could be arbitrarily large, this method works by recursively 
	 * calling itself after matching each attribute in order to account for this fact. If no more attributes appear, 
	 * it prints out the EPSILON production and exits. Additionally, it supports special error-checking by verifying 
	 * that no two attributes within a tag have the same name, which is done through a call of 
	 * <code>checkDuplicateNames()</code>. (Note that the parser terminates if any attributes are found to have 
	 * matching names!)
	 * 
	 * @throws ParserException if a syntax error is encountered 
	 */
	private void attribute() throws ParserException {
		if (predict(TokenType.NAME)) {		 
			...
			// Before exiting, clear the current name cache to account for any subsequent iterations 
	...
	/**
	 * Represents the grammar rules 'elementSuffix ::= > elementOrData endTag' and 'elementSuffix ::= />'.
	 * <p>
	 * It is through this method that the parser figures out if the current tag is an empty tag, in which case 
	 * the most recently cached name is discarded (since no name matching is necessary at this point). 
	 * 
	 * @throws ParserException if a syntax error is encountered
	 */
	private void elementSuffix() throws ParserException {
		if (predict(TokenType.CLOSE)) {	
			...
			// As the next token is />, then this must be an empty tag; no name matching necessary  
			if (!tagNames.isEmpty()) {
				tagNames.pop(); 
	 ...
	 * <p>
	 * The logic behind this method is that, within any given element in an XML-- document, there could be
	 * any arbitrary number of elements as well as data. As such, it recursively calls itself as long as 
	 * there are more elements or data to parse. Otherwise, the EPSILON terminator rule is applied to indicate 
	 * completion. 
	 *  
	 * @throws ParserException if a syntax error is encountered 
	 */
	 ...
	 * <p>
	 * Since this is an end tag, name-matching is performed here to verify that the name cached at the corresponding 
	 * start tag is identical. If a mismatch is detected, the parser displays the appropriate error message and 
	 * terminates.
	 * 
	 * @throws ParserException if a syntax error is encountered 
	 */
	private void endTag() throws ParserException {
		...
		matchTagName(this.tokens.get(1)); // second index as NAME is the second token that appears in the production
	 ...
	 * <p>
	 * This method is called frequently by <code>match()</code> to indicate that a legal token has been 
	 * consumed and the parser is ready to receive the next one, if any. 
	 */
	...
	/**
	 * Examines the lookahead token in order to deduce the production to be used to expand a non-terminal. 
	 * 
	 * @param type the type of the predicted token against which to check the lookahead
	 * @return <code>true</code> if the predicted token is equivalent to the current lookahead token, 
	...
	/**
	 * Consumes the current token (i.e. terminal) and advances to the next token in the token stream. 
	 * <p>
	 * For syntax checking purposes, this method also calls <code>matchNoAdvance()</code> to ensure that a token 
	 * is legal before it can be consumed. 
	 * 
	...
	/**
	 * Checks whether or not the current token is a legal symbol, without moving onto the next one. 
	 * <p>
	 * This method exists solely to be called by <code>match()</code> and cannot be used for consuming tokens. 
	 * @param type the type of the token predicted 
	 * @throws ParserException if an unexpected token is encountered 
	 */
	...
	private void cacheTagName(Token openName) throws ParserException {
		// Ensure that the token is in fact a NAME
		if (openName.getType() != TokenType.NAME) {
			throw new ParserException("Syntax error: An unexpected symbol has been encountered!"); 
		}
